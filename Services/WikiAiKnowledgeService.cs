...
public sealed class WikiAiKnowledgeService : IDisposable
{
    ...
    private static double Score(IReadOnlyList<string> terms, string content, string title, string space)
        => AiKnowledgeSearchService.CalculateRelevance(terms, content, title, space);
